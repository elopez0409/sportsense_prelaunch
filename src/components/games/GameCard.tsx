...
// Includes AI insight button for generating game analysis
...
export function GameCard({ game }: GameCardProps) {
  ...
  // Show AI insight button only for games that have started or finished (not SCHEDULED)
  const showAIButton = isLive || isHalftime || isFinal;
  ...
  // Handle AI button click - prevent navigation
  ...
  return (
    <>
        ...
        <Card className="group relative overflow-hidden hover:border-orange-500/50 transition-all duration-300 hover:shadow-orange-500/10 hover:shadow-2xl cursor-pointer">
          ...
          <div className="p-4 space-y-4">
            ...
            <div className="flex justify-between items-center">
              ...
              {game.nationalTv && (
                <span className="text-xs text-white/40 mr-8">{game.nationalTv}</span>
              )}
            </div>
      ...
      <AIInsightModal
        ...
        gameDate={new Date(game.scheduledAt).toISOString()}
      />
    </>
  );
}
